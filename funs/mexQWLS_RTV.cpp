...
int rowNum, colNum, rc, ccImg, ccImgMinusChaNum, ccImgPlusChaNum, chaNumImg, chaNumImgX2;
...
void getLaplacian(double **vecGuide, double **vecGuide_dual, double *a, double **b, double lambda, double sharpness, int sysLen);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //////// Q-WLS parameters  ///////////////
    double lambda = mxGetScalar(prhs[3]); // lambda
    double sharpness = mxGetScalar(prhs[4]); // sharpness
    r = (int)mxGetScalar(prhs[5]); // raius of the neighborhood
    int step = (int)mxGetScalar(prhs[6]);  // the step size
    ...
    double ***imgGuide = memAllocDouble3(rowNum, colNum, chaNumImg);
    ...
    double ***imgGuide_dual = memAllocDouble3(rowNum, colNum, chaNumImg);
    ...
    double **vecGuide = memAllocDouble2(maxSysLen, chaNumImg);  // RGB pixel values
    double **vecGuide_dual = memAllocDouble2(maxSysLen, chaNumImg);  // RGB pixel values
    ...
    getLaplacian(vecGuide, vecGuide_dual, a, b, lambda, sharpness, sysLen_col);
    ...
    getLaplacian(vecGuide, vecGuide_dual, a, b, lambda, sharpness, sysLen_row);
    ...
    for(col=r; col<maxIterColNum + step; col+=step)
    {
        ...
        {// column up-bottom extraction sliding direction
            for(row=0; row<rowNum; row++)
            {
                ...
                {// row left-to-right extraction sliding direction
                    ...
                    ptr_guide += ccImgPlusChaNum;
                    ptr_guide_dual += ccImgPlusChaNum;
                ...
                {// row right-to-left extraction sliding direction
                    ...
                    {
                        ptr_img -= chaNumImgX2;
                        ptr_guide -= chaNumImgX2;
                        ptr_guide_dual -= chaNumImgX2;
                        for(cha=0; cha<chaNumImg; cha++)
                        {
                            *ptr_vecImg++ = *ptr_img++;
                            *ptr_vecGuide++ = *ptr_guide++;
                    ...
                    ptr_guide += ccImgMinusChaNum;
                    ptr_guide_dual += ccImgMinusChaNum;
            ...
            if(leftright){
                ...
                ptr_guide -= ccImg - ptrShiftImg;
                ptr_guide_dual -= ccImg - ptrShiftImg;}
            else{
                ptr_img -= ccImgPlusChaNum + diaXChaImg - ptrShiftImg;
                ptr_guide -= ccImgPlusChaNum + diaXChaImg - ptrShiftImg;
                ptr_guide_dual -= ccImgPlusChaNum + diaXChaImg - ptrShiftImg;
        ...
        else // column column bottom-up extraction direction
        {
            ...
            if(leftright){
                ...
                ptr_guide += ccImgMinusChaNum - diaXChaImg + ptrShiftImg;
                ptr_guide_dual += ccImgMinusChaNum - diaXChaImg + ptrShiftImg;
                leftright = true;}
    ...
    for(row=r; row<maxIterRowNum + step; row+=step)
    {
        ...
        if(leftright) // row left-to-right extraction direction
        {
            ...
            if(updown){
                ...
                ptr_guide += ptrShiftImg - chaNumImg;
                ptr_guide_dual += ptrShiftImg - chaNumImg;}
            else{
                ptr_img -= chaNumImg + diaXCCImg - ptrShiftImg;
                ptr_guide -= chaNumImg + diaXCCImg - ptrShiftImg;
                ptr_guide_dual -= chaNumImg + diaXCCImg - ptrShiftImg;
...
void getLaplacian(double **vecGuide, double **vecGuide_dual, double *a, double **b, double lambda, double sharpness, int sysLen)
{
    ...
    double diffR, temp, weightR, eps, eps_s, valueMax;
    int i, j, k, n=sysLen;
    ...
    for(i=0; i<n-1; i++)
    {
        for(j=1; j<=min(r, n - 1 - i); j++)
        {
            ...
            for(k=0; k<chaNumImg; k++){
                ...
                diffR += temp * temp;}
            diffR = diffR / double(chaNumImg);
            valueMax = max(diffR, eps_s);
            weightR = 1 / valueMax;
            ...
            for(k=0; k<chaNumImg; k++){
                ...
                diffR += fabs(temp);}
            valueMax = max(diffR, eps);
            weightR *= 1 / valueMax;
            
            *ptr_b++ = -lambda * weightR;
            
            ptr_guide_cur -= chaNumImg;
            ptr_guide_cur_dual -= chaNumImg;
        }
        ...
        ptr_guide_cur += chaNumImg;
        ptr_guide_cur_dual += chaNumImg;
        ptr_guide_nei = ptr_guide_cur + chaNumImg;
        ptr_guide_nei_dual = ptr_guide_cur_dual + chaNumImg;
    }

...
void img2vec_col(float ***img, float ***imgGuide, float ***imgGuide_dual, float **vecImg, float **vecGuide, float **vecGuide_dual, int step);
void img2vec_row(float ***img, float ***imgGuide, float ***imgGuide_dual, float **vecImg, float **vecGuide, float **vecGuide_dual, int step);
void vec2img_col(float ***img, float **vec, int step);
void vec2img_row(float ***img, float **vec, int step);
void getLaplacian(float **vecGuide, float **vecGuide_dual, float *a, float **b, float lambda, float *rangeLUT, float *rangeLUT_dual, int sysLen);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    //////// Q-WLS parameters  ///////////////
    float lambda = mxGetScalar(prhs[3]); // lambda of the Q-WLS
    float sigmaR = mxGetScalar(prhs[4]); // range sigma for the guidance weight
    float sigmaR_dual = mxGetScalar(prhs[5]); // range sigma for the guidance weight
    ...
    float ***imgGuide_dual = memAllocfloat3(rowNum, colNum, chaNumGuide_dual);
    float *ptr_imgGuide_dual = &imgGuide_dual[0][0][0];
    for(int i=0; i<rowNum; i++)
        for(int j=0; j<colNum; j++)
    ...
    float **vecGuide = memAllocfloat2(maxSysLen, chaNumGuide);  // RGB pixel values
    float **vecGuide_dual = memAllocfloat2(maxSysLen, chaNumGuide_dual);  // RGB pixel values
    ...
    float *rangeLUT_dual = (float *)malloc(sizeof(float) * (maxRange_dual + 10));
...
void img2vec_col(float ***img, float ***imgGuide, float ***imgGuide_dual, float **vecImg, float **vecGuide, float **vecGuide_dual, int step)
{
    float *ptr_img, *ptr_guide, *ptr_guide_dual, *ptr_vecImg, *ptr_vecGuide, *ptr_vecGuide_dual;
...
void getLaplacian(float **vecGuide, float **vecGuide_dual, float *a, float **b, float lambda, float *rangeLUT, float *rangeLUT_dual, int sysLen)
{
    float *ptr_guide_cur, *ptr_guide_nei, *ptr_guide_cur_dual, *ptr_guide_nei_dual, *ptr_a, *ptr_b;
